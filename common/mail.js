...
exports.sendActiveMail = (who, token, name) => {
  ...
  const html =
    ...
    '<p>若您没有在' +
    config.name +
    '填写过注册信息，说明有人滥用了您的电子邮箱，请忽略或删除此邮件，我们对给您造成的打扰感到抱歉。</p>' +
    '<p>' +
    config.name +
    ' 谨上。</p>'
...
exports.sendNotificationMail = (
  ...
  const html =
    ...
    '<a href="' +
    SITE_ROOT_URL +
    '" target="_blank" style="text-decoration: none;"><h1 style="color: #3498db;">搞起博客</h1></a>' +
    '</div>' +
    '<div style="padding: 30px 40px 40px;">' +
    name +
    ' 您好, <a style="color: #3498db; text-decoration: none;" href="' +
    SITE_ROOT_URL +
    '/u/' +
    author +
    '" target="_blank">' +
    ...
    '<a style="color: #3498db; text-decoration: none; font-weight: bold;" href="' +
    SITE_ROOT_URL +
    '/p/' +
    (replyId ? [postId, '#', replyId].join('') : postId) +
    '" target="_blank">' +
    postTitle +
    '</a></div></div>' +
    '<div style="background: #EEE; border-top: 1px solid #DDD; text-align: center; height: 90px; line-height: 90px;"><a href="' +
    SITE_ROOT_URL +
    '/p/' +
    (replyId ? [postId, '#', replyId].join('') : postId) +
    '" style="padding: 8px 18px; background: #3498db; color: #FFF; text-decoration: none; border-radius: 3px;" target="_blank">查看详情 ➔</a></div></div><br>' +
    '</div>' +
    '</center>' +
    '</div>'
