 ...
 * @param replyId 评论id
 */
exports.sendNotificationMail = function (who, name, author, postTitle, postId, replyId) {
  var from = util.format('%s <%s>', config.name, config.mail_opts.auth.user)
  var to = who
  ...
  '<div style="border-left: 5px solid #DDD; padding: 0 0 0 24px; color: #888;">' +
    '<a style="color: #3498db; text-decoration: none; font-weight: bold;" href="' + SITE_ROOT_URL + '/p/' + (replyId ? [postId, '#', replyId].join('') : postId) + '" target="_blank">' + postTitle + '</a></div></div>' +
    '<div style="background: #EEE; border-top: 1px solid #DDD; text-align: center; height: 90px; line-height: 90px;"><a href="' + SITE_ROOT_URL + '/p/' + (replyId ? [postId, '#', replyId].join('') : postId) + '" style="padding: 8px 18px; background: #3498db; color: #FFF; text-decoration: none; border-radius: 3px;" target="_blank">查看详情 ➔</a></div></div><br>' +
  '</div>' +
  '</center>' +
  '</div>'
